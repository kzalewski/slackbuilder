...
# Last revised on 2016-03-28 - Added more doc filenames
#
...
if [ $skip_install_stage -eq 0 ]; then
  ...
  if [ $install_docs -eq 1 ]; then
    ...
    eval cp -v ABOUT* ANNOUNCE* announce.txt AUTHORS* BACKGROUND BUGS BUG-REPORTING bugs.txt BUILD* CHANGELOG CHANGELOG.md Change[Ll]og changelog.txt CHANGES* changes-* CODING* CodingStyle COMMIT* COMPAT* COMPILING* CONFIGURATION CONTRIBUTING CONTRIBUTING.md CONTRIBUTORS* COPYING* COPYRIGHT* CREDITS DEBUG DEVELOPERS Developers DISCLAIMER EXAMPLE* EXTENSIONS FAQ* FEATURES FILES FUTURE GOVERNANCE GOVERNANCE.md GPL HACKING HISTORY History* HOWTO* IDEAS INCOMPAT* INSTALL* INTRO KDE* KEYS KNOWN* LANGUAGE.HOWTO LANGUAGES LATEST.VER LEGAL LGPL* lgpl-license LICEN[CS]* License license.txt LSM MANIFEST MAPPING MIRRORS NAMING NEWS NOTES NOTICE OChangeLog ONEWS PACKAGING PERMISSIONS PKG-INFO PLATFORMS PORTING PORTING PORTS PROBLEMS QUICKSTART QuickInst README* Read[Mm]e* readme.txt RELEASE* ReleaseNotes* RELNOTES REVISION RFC.* ROADMAP ROADMAP.md Road[Mm]ap SOURCE SPONSORS STATUS THANKS THREADS TIPS TODO* ToDo todo.txt TRANSLATING TRANSLATORS TROUBLE* UPGRADING USING VERIFYING VERSION* WHATSNEW WHATS_NEW WhatsNew Y2KINFO $pkgname.[A-Z][A-Z]* $pkgname.spec $pkgname.lsm "$extra_doc_files" $destdir/$docdir 2>/dev/null
